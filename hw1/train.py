...
# -*- coding: utf-8 -*-
"""
Created on Sat Mar 10 16:52:29 2018
...
def load_test(path, transform=False):
    df_test = pd.read_csv(path, delimiter=',', header=None)
...
X, y = load_train(transform=True)
X = add_bias(X)

model = linear_regression_adgrad(lr=1, lda=0, max_itr=1e6)
model.fit(X, y, gd=True, verbose=0)
# model.evaluate(X, y)
# print(k_fold_cv(X, y, model, n_fold=9))

model.save_md(path='hw1_model.npy')
