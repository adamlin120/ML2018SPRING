...
X = np.c_[X, np.power(X[:, (1, 2, 5)], 2)]
X_test = np.c_[X_test, np.power(X_test[:, (1, 2, 5)], 2)]
...
#this_scores = cross_val_score(linearSVM, X, y, n_jobs=-1, cv=24)
#print('\nE_val= {}\tSTD= {}'.format(np.mean(this_scores), np.std(this_scores)))
